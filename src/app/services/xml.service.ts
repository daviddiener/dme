...
export class XMLService {
  ...
  constructor() { }
    ...
    public createArc (id: string, startId: string, targetId: string, textValue: string) {
        const parent = Global.xmlDoc
        .getElementsByTagName("pnml")[0]
        .getElementsByTagName("net")[0]
        .getElementsByTagName("page")[0]
    ...
    public updateNodePosition(id: string, x: number, y: number){
        Global.xmlDoc.querySelectorAll('[id="'+id+'"] graphics position')[0].setAttribute("x", x.toString())
        Global.xmlDoc.querySelectorAll('[id="'+id+'"] graphics position')[0].setAttribute("y", y.toString())
    }
