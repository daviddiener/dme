...
export class ModelExtractorComponent implements AfterViewInit {
    ...
    constructor(private ngZone: NgZone, private xmlService: XMLService) {}
    ...
    generateCardinalities() {
        ...
        Array.from(this.xmlService.getAllTransitions()).forEach((element) => {
            ...
            if (successor && predecessor) {
                const predecessorName =
                    this.xmlService.getNodeMarkingDataObjectName(
                        predecessor.getAttribute('source')
                    )
                const successorName =
                    this.xmlService.getNodeMarkingDataObjectName(
                        successor.getAttribute('target')
                    )
                ...
                if (predecessorName != successorName) {
                    this.addRelation(
                        ...
                        successorName,
                        String(
                            predecessor.getElementsByTagName('hlinscription')[0]
                                .textContent
                        ),
                        String(
                            successor.getElementsByTagName('hlinscription')[0]
                                .textContent
                        )
                    )
                }
        ...
        console.log(
            '=== Creating relation from transition role assignments ==='
        )
        ...
        Array.from(this.xmlService.getTransitionOwners()).forEach((element) => {
            ...
            */
            this.xmlService
                .getAllArcsWithSource(element.getAttribute('id'))
                .forEach((arc) => {
                    const sourceName = String(
                        element
                            .getElementsByTagName('owner')[0]
                    ...
                    const sourceRef = this.relationReferenceList.find(
                        (el) => el.startNode.textValue == sourceName
                    )
                    const targetRef = this.relationReferenceList.find(
                        (el) => el.targetNode.textValue == targetName
                    )
                    ...
                    if (!(sourceRef && targetRef)) {
                        ...
                        console.log(
                            'Skipping because of duplicate Relation from ' +
                                sourceName +
                                ' to ' +
                                targetName
                        )
    ...
    generateClassesFromMarkings() {
        ...
        Array.from(this.xmlService.getAllPlaces()).forEach((place) => {
            ...
            if (marking.length > 0) {
                ...
                if (
                    this.classReferenceList.find(
                        (el) => el.textValue == marking[0].getAttribute('name')
                    ) == undefined
                ) {
                    // set sprite tint to red if the class is an existing (external) object
                    let color = 0xff0000
                    if (
                        objectsWithIncomingArcs.some(
                            (x) => x == String(place.getAttribute('id'))
                        )
                    ) {
                        color = 0xffffff
                    }
    ...
    addRelation(
        ...
        const sourceRef = this.classReferenceList.find(
            (el) => el.textValue == sourceName
        )
        const targetRef = this.classReferenceList.find(
            (el) => el.textValue == targetName
        )
        ...
        if (sourceRef && targetRef) {
            ...
            console.log(
                'created relation from ' + sourceName + ' to ' + targetName
            )
        }
        return tmpRelation
    }
